...
### 新增后续任务：敌人资源级动画第二轮与攻击前摇规范
状态：in_progress

内容：
- 史莱姆补资源级/程序级动作：吐酸前膨胀、吐酸发射、受击回弹、死亡软体坍缩或淡出。（史莱姆程序动画第二轮已完成烟测）
- Boss 补专属动作：登场、普通攻击、技能前摇、受击、死亡，避免只用通用怪物动作。
- 精英怪做颜色与动画差异：材质/描边/粒子颜色区分，同时用更强的呼吸、蓄力、移动节奏和受击反馈强化威胁感。
- 统一敌人攻击前摇规范：所有敌人在造成伤害前必须有清晰可读的蓄力姿态、方向提示、危险窗口和恢复段。（第一轮已完成烟测）

验收标准：
- 玩家能在攻击命中前读懂史莱姆吐酸、骷髅冲锋、Boss 技能和精英怪强化攻击。
- 不同敌人类型在移动、攻击、受击、死亡时有明显差异。
- 敌人前摇时长、提示强度和命中窗口可被 QA_Sandbox 快速触发验证。
- 不改变现有敌人伤害、生成、掉落和波次逻辑，优先增强表现层。

验收记录：
- 已为 `CharacterAnimationController` 增加 Slime 专属吐酸、受击、死亡程序动画曲线。
- QA 面板已暴露 Slime Spit Swell、Slime Hurt Squash、Slime Death Melt 三个调参滑条。
- Slime 死亡时先禁用碰撞/NavMeshAgent，保留模型播放软体坍缩，再延迟隐藏与销毁，避免死亡瞬间消失。
- UnityMCP 编译刷新通过；QA_Sandbox Play Mode 烟测生成 Slime、触发吐酸、受击、击杀后 Console Error/Warning 为 0。
- 已盘点当前资源：项目内没有 Slime/Blob/Gel/Monster 相关 FBX/AnimationClip，当前无法绑定 Slime 资源级骨骼动画；Slime 仍以程序动画为主。
